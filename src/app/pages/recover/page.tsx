...
export default function messageRecover() {
    ...
    const handleRecover = () => {
        if (!mail) {
            ...
            authService.recover(mail).then((res) => {
                setMail('')
                setSuccessrMessage('Revisa tu correo para verificar el código')
            })
        }
    }
    return (
        <Container title="¿Olvidaste tu contraseña?" showButtons={false} showContainer={true}>
            <div className="row justify-content-center" >
                ...
                <div>
                    ...
                    <div className="card flex flex-wrap align-items-center justify-content-center gap-3">
                        ...
                        {successMessage && (<Message severity="info" text={successMessage} />)}
                    </div>
                </div>
            </div>
